...
change_VOC <- function(
  ...
  rc_severity = c(1.5,1.5), # relative change in ihr and ifr
  rc_transmissibility = c(1.5, 1.5), # relative change in transmissibility via 
  # u, uv and uv2
  rc_ve = c(0.5, 0.5) # relative in ve against infection
){
  ...
  burden_processes_new <- 
    list(cm_multinom_process("E",       
         ...
         cm_multinom_process("E",       
                             data.frame(death_voc2 = P.death*prod(rc_severity)),                   
                             delays = data.frame(death_voc = delay_2death), 
                             report = "o"),
         ...
         cm_multinom_process("Ev",      
                             ...
                             report = "o"),
         cm_multinom_process("Ev",      
                             data.frame(death_voc2 = P.death*(1-ve$ve_mort[1])*prod(rc_severity)), 
                             delays = data.frame(death_voc = delay_2death), 
                             report = "o"),
         ...
         cm_multinom_process("Ev2",     
                             ...
                             report = "o"),
         cm_multinom_process("Ev2",     
                             data.frame(death_voc2 = P.death*(1-ve$ve_mort[2])*prod(rc_severity)), 
                             delays = data.frame(death_voc = delay_2death), 
                             report = "o"),
         ...
         cm_multinom_process("E",       
                             data.frame(to_hosp_voc2 = P.hosp*prod(rc_severity)),                  
                             delays = data.frame(to_hosp_voc = delay_2severe)),
         ...
         cm_multinom_process("Ev",      
                             ...
                             delays = data.frame(to_hosp_voc = delay_2severe)),
         cm_multinom_process("Ev",      
                             data.frame(to_hosp_voc2 = P.hosp*(1-ve$ve_h[1])*prod(rc_severity)),   
                             delays = data.frame(to_hosp_voc = delay_2severe)),
         ...
         cm_multinom_process("Ev2",     
                             ...
                             delays = data.frame(to_hosp_voc = delay_2severe)),
         cm_multinom_process("Ev2",     
                             data.frame(to_hosp_voc2 = P.hosp*(1-ve$ve_h[2])*prod(rc_severity)),   
                             delays = data.frame(to_hosp_voc = delay_2severe)),
  ...
  data.table(u   =  para$pop[[1]]$u,
    ...
    mutate(ve_i  =  1 - uv/u,
           ve_i2 =  1 - uv2/u) %>%
    mutate(u_ds1 = u*rc_transmissibility[1],
           u_ds2 = u*prod(rc_transmissibility),
           ve_i_ds1 = ve_i*rc_ve[1],
           ve_i_ds2 = ve_i*prod(rc_ve),
           ve_i2_ds1 = ve_i*rc_ve[1],
           ve_i2_ds2 = ve_i*prod(rc_ve),
           uv_ds1 = u_ds1*(1-ve_i_ds1),
           uv_ds2 = u_ds2*(1-ve_i_ds2),
           uv2_ds1 = u_ds1*(1-ve_i2_ds1),
           uv2_ds2 = u_ds2*(1-ve_i2_ds2)) -> u_table
  ...
  para$schedule[[paste0("change_uv2")]] <-  list(
    ...
    values = list(u_table %>% pull(uv2), 
                  u_table %>% pull(uv2_ds1),
                  u_table %>% pull(uv2_ds2)),
    times = c(para$date0, date_switch)
  )
...
vac_policy <- function(para,
  ...
  draw_by_switch <- function(switch){
    daily_vac_scenarios <-
      matrix(0, 
    ...
    t_switch <- daily_vac_scenarios %>% filter(date == switch) %>% 
      pull(t) %>% as.numeric
    ...
    data.table(t_dose1 = as.numeric(NA), 
               t_dose2 = as.numeric(NA), 
               status = as.character(NA), 
               elapse = as.numeric(NA)) %>% 
      bind_cols(daily_vac_scenarios %>% 
                  dplyr::select(ends_with("_d1")) %>% 
                  .[1,]) %>% 
      mutate_at(vars(starts_with("Y", ignore.case = F)), 
                function(x) x = as.numeric(NA))-> pending
    
    # complete vaccinating the first priority
    for(t in vacc_start:(t_switch-1)){
      # for(t in vacc_start:740){
      # whether or not we allocate dose 2
      switch_dose2 <- any(pending$elapse >= dose_interval*7 & pending$status == "incomplete", na.rm = T)
      # saturation status
    ...
    for(t in t_switch:nrow(daily_vac_scenarios)){
    # for(t in t_switch:(t_switch + 10)){
      ...
      # if everyone with priority = 1 has been vaccinated with d1
      # move on to vaccinate priority = 2 with d1
      if(saturated[priority == 1 & dose == "d1"]$saturated == T &
      ...
      if(saturated[priority == 1 & dose == "d1"]$saturated == T &
        ...
        # if everyone has been vaccinated with d1
        # move on to vaccinate priority = 1 with d2
        if(saturated[priority == 1 & dose == "d1"]$saturated == F &
           saturated[priority == 1 & dose == "d1"]$saturated == F){
          v <- doses_available*tmp_pop_prop[[1]]; daily_vac_scenarios[t, tmp_tar2[[1]] := as.list(v)]; rm(v) 
          
          # identify the ones that are available to receive the second dose
          pending %>% filter(elapse > dose_interval & status == "incomplete") %>% arrange(desc(elapse)) %>%
      ...
      # elapse everyone in pending by one unit if they remain "incomplete"
      pending %<>% 
        mutate(elapse = if_else(status == "incomplete", elapse + 1, elapse)) %>% 
        replace(., .<0, 0)
  ...
  scenarios <- switch_date %>% map(draw_by_switch)
      ...
      if(!is.na(switch_dose2) & switch_dose2 == T){
        ...
        if(doses_available < doses_needed){
          ...
          tmp %>%
            ...
            pull(ag) %>% unique -> ag_involved
          
          cross_ag <- (8 %in% ag_involved) & (15 %in% ag_involved)
  ...
  scenarios[[(length(scenarios) + 1)]] <- draw_rollout_interval(dose_interval)
